...
class Coach:
    def __init__(self, opts):
        ...
        # Initialize network
        self.net = StyleCLIPMapper(self.opts)
        self.net.cuda(self.opts.gpu)
        self.net = torch.nn.parallel.DistributedDataParallel(
            self.net, device_ids=[self.opts.gpu], broadcast_buffers=False).module

        # Initialize loss
        if self.opts.id_lambda > 0:
            self.id_loss = id_loss.IDLoss(self.opts).cuda(self.opts.gpu).eval()
        if self.opts.clip_lambda > 0:
            self.clip_loss = clip_loss.CLIPLoss(self.opts).cuda(self.opts.gpu).eval()
        if self.opts.latent_l2_lambda > 0:
            self.latent_l2_loss = nn.MSELoss().cuda(self.opts.gpu).eval()
        ...
        train_dataset, test_dataset = self.configure_datasets()
        # Distributed sampler
        ...
        self.train_dataset_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=self.opts.batch_size, sampler=train_dataset_sampler,
            num_workers=int(self.opts.workers), drop_last=True)
        test_dataset_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset)
        self.test_dataset_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=self.opts.test_batch_size, sampler=test_dataset_sampler,
            num_workers=int(self.opts.workers), drop_last=True)
        ...
        # Initialize logger
        if self.opts.multiprocessing_distributed and self.opts.gpu == 0:
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            ...
            for batch_idx, batch in enumerate(self.train_dataset_loader):
                print(f'batch_idx = {batch_idx}, batch.shape = {batch.shape}')
                count += len(batch)
                self.optimizer.zero_grad()
                if self.opts.work_in_stylespace:
                    w = convert_s_tensor_to_list(batch)
                    w = [c.cuda(self.opts.gpu) for c in w]
                else:
                    w = batch
                    w = w.cuda(self.opts.gpu)
                with torch.no_grad():
                ...
                self.global_step += 1
            print(f'GPU_{self.opts.gpu}, Step_{self.global_step}, data_count = {count}')

    def validate(self):
        self.net.eval()
        agg_loss_dict = []
        for batch_idx, batch in enumerate(self.test_dataset_loader):
            if batch_idx > 200:
                break
    ...
    def configure_datasets(self):
        if self.opts.latents_train_path:
            train_latents = torch.load(self.opts.latents_train_path).cuda(self.opts.gpu)
        else:
            mean_latent = self.net.decoder.mean_latent(4096).cuda(self.opts.gpu)
            train_latents_z = torch.randn(self.opts.train_dataset_size, 512).cuda(self.opts.gpu)
            train_latents = []
            for b in range(self.opts.train_dataset_size // self.opts.batch_size):
                with torch.no_grad():
                    # _, train_latents_b, _ = self.net.decoder([train_latents_z[b * self.opts.batch_size: (b + 1) * self.opts.batch_size]],
                    # 									  truncation=0.7, truncation_latent=self.net.latent_avg, return_latents=True)
                    _, train_latents_b, _ = \
                        self.net.decoder([train_latents_z[b * self.opts.batch_size: (b + 1) * self.opts.batch_size]],
                                         truncation=0.7, truncation_latent=mean_latent, return_latents=True)
                    train_latents.append(train_latents_b)
            train_latents = torch.cat(train_latents)

        if self.opts.latents_test_path:
            test_latents = torch.load(self.opts.latents_test_path).cuda(self.opts.gpu)
        else:
            mean_latent = self.net.decoder.mean_latent(4096).cuda(self.opts.gpu)
            test_latents_z = torch.randn(self.opts.train_dataset_size, 512).cuda(self.opts.gpu)
            test_latents = []
            for b in range(self.opts.test_dataset_size // self.opts.test_batch_size):
                with torch.no_grad():
                    # _, test_latents_b, _ = self.net.decoder([test_latents_z[b * self.opts.test_batch_size: (b + 1) * self.opts.test_batch_size]],
                    # 								  truncation=0.7, truncation_latent=self.net.latent_avg, return_latents=True)
                    _, test_latents_b, _ = self.net.decoder(
                        [test_latents_z[b * self.opts.test_batch_size: (b + 1) * self.opts.test_batch_size]],
                        truncation=0.7, truncation_latent=mean_latent, return_latents=True)
